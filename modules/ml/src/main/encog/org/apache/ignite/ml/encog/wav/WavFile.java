...
package org.apache.ignite.ml.encog.wav;// Wav file IO class
// A.Greensted
...
public class WavFile
{
	...
	private int bytesPerSample;			// Number of bytes required to store a single sample.wav
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				// Calculate the number of bytes required to hold 1 sample.wav
	...
	public void display(PrintStream out)
	{
		...
		out.printf("Valid Bits: %d, Bytes per sample.wav: %d\n", validBits, bytesPerSample);
	}
